...
<div class="render">
    <div class="title">渲染设置
        <span class="switch" id="open" onclick="$(this).hide();$('#close').show();$('.trender').show();">展开</span>
        <span class="switch" id="close" style="display:none;" onclick="$(this).hide();$('#open').show();$('.trender').hide();">关闭</span>
    </div>
    <table class="trender" style="display:none;">
        <tr><td colspan="10" style="font-size:12px;">注意：这里统一16/9格式</td></tr>
        <tr>
        ...
        <tr>
            <td colspan="3" style="border:0;">
                <a class="getSubmit" title="点击开始渲染" onclick="toRender()">确定渲染成片</a>
            </td>
        </tr>
        <tr><td colspan="5" style="height:10px;border:0;"></td></tr>
    </table>
...
<script>
    ...
    function toRender(){
        ...
        var renderMoney = $("select[name='format']").val();
        var totalMoney = editMoney*1+addMoney*1+renderMoney*1;
        window.location.href = '{{DOMAIN}}online/u/order/apply/'+productid+''+renderMoney+'/'+totalMoney;
    }
</script>