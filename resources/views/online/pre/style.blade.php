...
/*================================*/
    ...
    /*动画的代码开始：定义动画时间*/
    @if(count($layers))
    @foreach($layers as $klayer=>$vlayer)
    {{'.'.$vlayer->getAttrStyleName()}} div.dh {
            animation-name:{{$vlayer->a_name}};
            animation-play-state:paused;
            animation-duration:{{$vlayer->timelong}}s;
            animation-timing-function:ease;
            animation-delay:{{$vlayer->delay}}s;
    ...
    animation-play-state:paused;
    animation-duration:{{$layers[count($layers)-1]->delay + $layers[count($layers)-1]->timelong}}s;
