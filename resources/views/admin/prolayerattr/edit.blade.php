    ...
    <div class="admin-content">
        ...
        <div class="am-g">
            ...
            <div class="am-u-sm-12 am-u-md-8 am-u-md-pull-4">
                <form class="am-form" data-am-validator method="POST" action="{{DOMAIN}}admin/{{$layerModel->id}}/prolayerattr/{{ $data->id }}" enctype="multipart/form-data">
                    ...
                    <fieldset>
                            ...
                            <select name="attrSel" required>
                                @if($model['attrSels'])
                                    @foreach($model['attrSels'] as $kattrSel=>$attrSel)
                                        <option value="{{ $kattrSel }}" {{ $data->attrSel==$kattrSel ? 'selected' : '' }}>{{ $attrSel }}</option>
                                    @endforeach
                                @endif
                            </select>
                        </div>
                        ...
                        <div class="am-form-group">
                            <label>动画值 / Val：</label>
                            <input type="text" placeholder="关键值" minlength="1" required name="val" value="{{ $data->val }}"/>
                        </div>
