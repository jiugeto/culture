                ...
                <form class="am-form" data-am-validator method="POST" action="{{DOMAIN}}admin/ad/{{ $data->id }}" enctype="multipart/form-data">
                    ...
                    <fieldset>
                        ...
                        <div class="am-form-group">
                            ...
                            <input type="hidden" pattern="^\d+$" name="pic_id" value="{{ $data->pic()->id }}"/>
                            <input type="text" placeholder="选择图片" minlength="2" name="pic_name" value="{{ $data->pic()->name.'('.$data->pic()->width.'*'.$data->pic()->height.')' }}"/>
                            <div class="pic_list">
                                @if(count($pics))
                                    @foreach($pics as $pic)
                                <div class="img {{ $data->pic()->id==$pic->id ? 'curr' : '' }}"
                                     onclick="getPic('{{ $pic->id.'-'.$pic->name.'-'.$pic->width.'-'.$pic->height }}')"
                        ...
                        <div class="am-form-group">
                            ...
                            <input type="text" placeholder="点击选择日期" class="form-datetime am-form-field" data-am-datepicker="{format: 'yyyy-mm-dd'}" name="fromTime" value="{{$data->fromTime()}}"/>
                        </div>

                        <div class="am-form-group">
                            <label>有效时间结束 / End：</label>
                            <input type="text" placeholder="点击选择日期" class="form-datetime am-form-field" data-am-datepicker="{format: 'yyyy-mm-dd'}" name="toTime" value="{{ $data->toTime() }}"/>
                        </div>
