    ...
    <div class="per_body" style="border:0;background:0;">
        ...
        <div class="per_left">
            ...
            <div class="per_left3">
                ...
                <div class="img">
                    @if($pic->url)
                    <img src="{{ $pic->getUrl() }}" style="@if($size=$pic->getPicSize($pic,$w=55,$h=50)) width:{{$size['w']}}px;height:{{$size['h']}}px; @endif">
                    @else
        ...
        <div class="per_center">
            <div class="per_center1">
                ...
                <table cellpadding="0" cellspacing="0" width="100%">
                    <tr>
                        ...
                        <td>真实姓名：{{ $user['person']['realname'] }}</td>
                    </tr>
                    <tr>
                        <td>性别：{{ $user['person']['sexName'] }}</td>
                        ...
                        <div class="img">
                            @if($good->getPicUrl())
                                <img src="{{ $good->getPicUrl() }}" style="
                                    @if($size=$good->getPicSize($w=100,$h=50)) width:{{$size['w']}}px;height:{{$size['h']}}px; @endif
                                    ">
                            @else
                ...
                <a href=""><div class="video">
                    <div class="img">
                        @if($product->getPicUrl())
                            <img src="{{ $product->getPicUrl() }}" style="
                                @if($size=$product->getPicSize($w=100,$h=50)) width:{{$size['w']}}px;height:{{$size['h']}}px; @endif
                                ">
                        @else
                            <div style="margin:0;width:100px;height:50px;background:rgb(240,240,240);border:0;"></div>
                        @endif
            ...
            <div class="per_center3">
                ...
                <div class="message">
                    ...
                    <p>来自 {{ $message->senderName() }} &nbsp;&nbsp;&nbsp;&nbsp; 发送于 {{ $message->createTime() }}</p>
                </div>
                    @endforeach
                @else <p style="text-align:center;line-height:50px;color:grey;">没有视频</p>
                @endif
        ...
        <div class="per_right">
            <div class="per_right1">
                ...
                <div class="friend">
                    ...
                    <div class="img">
                        @if($frield['user_pic'])
                            <div class="imgHidden">
                                <img src="{{$model->getUrlByPicid($frield['user_pic'])}}" style="
                                    @if($size=$model->getImgSize($frield['user_pic'],$w=50,$h=50))
                    ...
                    @else
                    <div class="img">
                        @if($frield['frield_pic'])
                            <div class="imgHidden">
                                <img src="{{$model->getUrlByPicid($frield['frield_pic'])}}" style="
                                    @if($size=$model->getImgSize($frield['frield_pic'],$w=50,$h=50))
                                        width:{{$size['w']}}px;height:{{$size['h']}}px;
                                    @endif
                                    ">
                            </div>
            ...
            <div class="per_right3">
                ...
                <div class="img">
                    @if($design->thumb)
                        <div class="imgHidden">
                            <img src="{{$model->getUrlByPicid($design->thumb)}}" style="
                                @if($size=$model->getImgSize($design->thumb,$w=80,$h=100))
                                    width:{{$size['w']}}px;height:{{$size['h']}}px;
                                @endif
                                ">
                        </div>
