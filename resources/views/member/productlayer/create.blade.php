...
@section('content')
    ...
    <form data-am-validator method="POST" action="{{DOMAIN}}member/productlayer" enctype="multipart/form-data">
        ...
        <table class="table_create">
            ...
            <tr>
                ...
                <td><input type="text" class="field_value" placeholder="至少2个字符" pattern="^[a-zA-Z0-9-_]+$" required name="animation_name"/></td>
            </tr>
            {{--<tr><td></td></tr>--}}

            <tr>
                <td class="field_name"><label>时长：</label></td>
                <td><input type="text" class="field_value" placeholder="" pattern="^\d+$" required name="duration"/></td>
            </tr>
            {{--<tr><td></td></tr>--}}
            ...
            <tr>
                ...
                <td><input type="text" class="field_value" placeholder="" pattern="^\d+$" required name="delay"/></td>
            </tr>
            {{--<tr><td></td></tr>--}}

            <tr>
                <td class="field_name"><label>播放次数：</label></td>
                <td><input type="text" class="field_value" placeholder="" pattern="^([1-9])|([1-9]\d+)$" required name="count"/></td>
            </tr>
            {{--<tr><td></td></tr>--}}
                ...
                <td>
                    <select name="direction">
                        @foreach($model['directionNames'] as $kdirection=>$directionName)
                            <option value="{{ $kdirection }}">{{ $directionName }}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
                ...
                <td>
                    <select name="state">
                        @foreach($model['stateNames'] as $kstate=>$stateName)
                            <option value="{{ $kstate }}">{{ $stateName }}</option>
                        @endforeach
                    </select>
                </td>
            </tr>
