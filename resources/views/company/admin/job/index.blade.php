        ...
        <table class="table_create" cellspacing="0" cellpadding="0">
            @if(isset($data->jobs) && isset($data->jobRequires) && $data->jobs && $data->jobRequires)
                ...
                <tr>
                    ...
                    <td>
                        <a href="/company/admin/job/{{$data->id}}-{{ $kjob }}/edit" class="job">修改</a>
                        <a href="/company/admin/job/{{$data->id}}-{{ $kjob }}/del" class="job">删除</a>
                    </td>
                </tr>
                @endforeach
                <tr><td colspan="2">&nbsp;</td><td><a href="/company/admin/job/create/{{$data->id}}" class="job">新加岗位</a></td></tr>
            @else <tr><td colspan="2">无招聘需求</td><td><a href="/company/admin/job/create/{{$data->id}}" class="job">新加岗位</a></td></tr>
            @endif
